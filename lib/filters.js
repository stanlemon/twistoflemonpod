...
function findTranscript(collections, episode) {
  ...
  return (
    collections.transcripts.find((t) => t.data.episode === episode) || null
  );
}
...
function excludeFromFeed(content) {
  ...
  return content.replace(
    /<!--\s*feed-exclude-start\s*-->[\s\S]*?<!--\s*feed-exclude-end\s*-->/g,
    "",
  );
}

/**
 * Normalize a URL to canonical form by removing query strings and hash fragments
 * @param {string} url - URL to normalize (can be relative or absolute)
 * @param {string} baseUrl - Base URL to prepend if url is relative (optional)
 * @returns {string} Canonical URL
 */
function canonicalUrl(url, baseUrl = "") {
  // If no URL is provided, fall back to the site root
  if (!url) {
    if (!baseUrl) return "/";
    const base = baseUrl.replace(/\/$/, "");
    return `${base}/`;
  ...
  // If path is already absolute (contains ://), return it as-is
  if (path.includes("://")) {
    return path;
  }

  // Normalize relative paths to have a leading slash
  if (!path.startsWith("/")) {
    path = `/${path}`;
  }

  // If baseUrl is provided, prepend it (stripping any trailing slash)
  if (baseUrl) {
    const base = baseUrl.replace(/\/$/, "");
    return base + path;
  }
