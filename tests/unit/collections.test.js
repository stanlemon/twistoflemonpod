...
describe("Collections: buildNormalizedCollection", () => {
  ...
  it("sorts posts by date (newest first)", () => {
    ...
    assert.strictEqual(result.tech.posts.length, 3);
    assert.strictEqual(
      result.tech.posts[0].date.getTime(),
      new Date("2024-01-05").getTime(),
    );
    assert.strictEqual(
      result.tech.posts[1].date.getTime(),
      new Date("2024-01-03").getTime(),
    );
    assert.strictEqual(
      result.tech.posts[2].date.getTime(),
  ...
  it("handles multiple values per item", () => {
    const items = [
      {
        data: { categories: ["Tech", "Podcast", "Life"] },
...
describe("Collections: buildTagsCollection", () => {
  ...
  it("excludes future-dated posts by default", () => {
    ...
    assert.strictEqual(result.javascript.posts.length, 1);
    assert.strictEqual(
      result.javascript.posts[0].date.getTime(),
      new Date("2024-01-01").getTime(),
    );
  });
